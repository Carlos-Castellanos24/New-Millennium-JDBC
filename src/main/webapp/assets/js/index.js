...
function validarFacilitador(texto, campos_alerta, alerta_nombre, longitud) {
    ...
    if (texto.value.length < longitud) {
        document.getElementById(alerta_nombre).innerHTML = campos_alerta + " debe tener mas de " + longitud + " caracteres. Actualmente cuenta con: " + (texto.value.length + 1);
        desabilitar.disabled = true;
    } else {
...
function validarCategoria(texto, campos_alerta, alerta_descripEdit, longitud) {
    ...
    if (texto.value.length < longitud) {
        document.getElementById(alerta_descripEdit).innerHTML = campos_alerta + " debe tener mas de " + longitud + " caracteres. Actualmente cuenta con: " + (texto.value.length + 1);
        desabilitar.disabled = true;
    } else {
