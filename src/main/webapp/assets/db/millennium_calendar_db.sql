...
-- [TABLA ADMINISTRADOR 1/6] INSERT
USE millennium_calendar_db;
...
-- [TABLA ADMINISTRADOR 2/6] UPDATE
USE millennium_calendar_db;
...
-- [TABLA ADMINISTRADOR 3/6] SELECT
USE millennium_calendar_db;
DROP PROCEDURE IF EXISTS SP_listar_administrador;
CREATE PROCEDURE SP_listar_administrador()
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY
DEFINER
SELECT a.*, l.* FROM administrador a INNER JOIN login l ON a.id_administrador = l.id_administrador ORDER BY a.estado_administrador ASC;

-- [TABLA ADMINISTRADOR 4/6] SELECT BY ID
USE millennium_calendar_db;
...
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY
DEFINER
SELECT administrador.id_administrador ,login.id_login , administrador.nombre, administrador.apellido, correo, estado_administrador FROM administrador INNER JOIN login 
on administrador.id_administrador = login.id_administrador
where id_administrador2 = login.id_administrador;

-- [TABLA ADMINISTRADOR 5/6] DELETE
USE millennium_calendar_db;
...
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY
DEFINER
UPDATE administrador INNER JOIN login ON(administrador.id_administrador = login.id_administrador)
SET administrador.estado_administrador = 'I', login.estado_login = 'I'
WHERE administrador.id_administrador = id_administrador2;

-- [TABLA ADMINISTRADOR 6/6] ULTIMO ADMINISTRADOR AGREGADO
DROP PROCEDURE IF EXISTS SP_obtener_administrador;
CREATE PROCEDURE SP_obtener_administrador(
)
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY 
DEFINER
SELECT id_administrador FROM administrador ORDER by id_administrador DESC LIMIT 1;

-- --------------------------------------
-- [SP TABLA FACILITADOR]
-- [TABLA FACILITADOR 1/6] INSERT
USE millennium_calendar_db;
...
-- [TABLA FACILITADOR 4/6] SELECT COMPLETO
USE millennium_calendar_db;
DROP PROCEDURE IF EXISTS SP_listar_facilitador_completo;
CREATE PROCEDURE SP_listar_facilitador_completo()
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY
DEFINER
SELECT * FROM facilitador ORDER BY estado_facilitador ASC;

-- [TABLA FACILITADOR 5/6] SELECT BY ID
USE millennium_calendar_db;
...
-- [TABLA CATEGORIA 4/6] SELECT COMPLETO
USE millennium_calendar_db;
DROP PROCEDURE IF EXISTS SP_listar_categorias_completo;
CREATE PROCEDURE SP_listar_categorias_completo()
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY 
DEFINER SELECT * FROM categoria ORDER BY estado_categoria ASC;

-- [TABLA CATEGORIA 5/6] SELECT BY ID 
USE millennium_calendar_db;
...
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY
DEFINER 
UPDATE login SET id_administrador = id_administrador2, correo = correo2, estado_login = estado_login2 WHERE id_login = id_login2;
...
-- [TABLA ACTIVIDAD 6/8] SELECT ADMINISTRADOR
USE millennium_calendar_db;
...
-- [TABLA ADMINISTRADOR 8/8] ULTIMO ADMINISTRADOR AGREGADO
DROP PROCEDURE IF EXISTS SP_obtener_actividad;
CREATE PROCEDURE SP_obtener_actividad(
)
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY 
DEFINER
SELECT id_actividad FROM actividad ORDER by id_actividad DESC LIMIT 1;
...
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY
DEFINER
INSERT INTO administrador_actividad VALUES('NULL', id_administrador, id_actividad, CURRENT_TIMESTAMP());
...
NOT DETERMINISTIC CONTAINS SQL SQL SECURITY
DEFINER
UPDATE administrador_actividad SET id_administrador = id_administrador2, id_actividad = id_actividad2 WHERE id_proceso = id_proceso2;
...
CALL SP_agregar_administrador('Conny Alexandra','Ventura','A');
...
-- [TABLA LOGIN]
-- Calve: holamundo@
CALL SP_agregar_login (1,'melissa@mail.com','09d4c2a195336116e6d57b279f890b34','A');
-- Calve: holamundo#
CALL SP_agregar_login (2,'diego@mail.com','05b5eeea2613becdc7bfa00c925df40d','A');
-- Calve: holamundo$
CALL SP_agregar_login (3,'rodolfo@mail.com','0c8786040d73ff78fdbc6df71a907820','I');
-- Calve: holamundo!
CALL SP_agregar_login (4,'conny@mail.com','5c88ff8f2556d586ca047330b4b561a4','A');
...
CALL SP_agregar_administrador_actividad(1,1);
